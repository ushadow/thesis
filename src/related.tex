...
system into several sequential modules shown in Figure \ref{fig:pipeline}, and
discuss the related work in each module.
\begin{figure}[tbh]
  \centering
...
New generation of sensors. Mime~\cite{colaco13} is a compact, low-power 3D
sensor for single-handed gestural interaction. It combines a three-pixel
time-of-flight (TOF) module with a standard RGB camera.
...
gestures, though some efforts have attempted to handle multiple types.
...
with distinct paths \cite{Starner95, sharma00}. 

Starner and Pentland used HMMs
for sign language recognition without considering finger
spelling~\cite{Starner95}. They collected about 500 sentences of a specific grammar (``personal pronoun,
verb, noun, adjective, (the same) personal pronoun'') with a total lexicon of
...
sentence, we embed gesture phase HMMs to identify different gesture sub-phases which is necessary for real-time interaction.
