...
Our society is transforming itself to use the power of computers throughout its entire fabric - wherever information is used - and that transformation depends critically on the quality of human-computer interaction \cite{Card83}. Computers have become ubiquitous, be it in office, at home, in commercial areas or in science laboratories. The amount of time people interact with computers has increased tremendously. 

Following Moore's law, there is roughly an exponential rate of improvement on computer hardware in terms of processing speed and memory capacity in the past few decades. Meanwhile, display technology has also had significant advancement. Large displays and small pocket-size portable displays are two major trends of the development. However, the predominant mode of human computer interaction has not changed substantially since the creation of the windows, icons, menus, and pointer more than thirty years ago, interfaces that themselves rely on technology going back forty years and more.  This useful but limited paradigm has both locked us into a single mindset about interaction, and homogenized the physical performance of any computer-related work.  In many ways, they are still convenient and efficient ways for interaction. But with new displays and a growing demand for richer interaction, they appear increasingly limited. For example, using a mouse with a large display will be very cumbersome due to the large distance to move. A mouse is also limited to 2D interaction. As interaction moves away from the confinement of desktop, the use of a keyboard becomes cumbersome too.

Some new interaction techniques have emerged to provide novel ways of interaction (e.g., the Microsoft Surface\textsuperscript{\textregistered}, the Nintendo\textsuperscript{\textregistered} Wii, iPhone\textsuperscript{\textregistered}), with a common aspiration of making interacting with computation more effortless. Our goal is to take this aspiration to the next level by developing an intelligent multimodal interface for natural interaction. By \textit{natural interaction}, we mean the kind of cognitively transparent, effortless multimodal communication that can happen between people; we want to make this possible in human-computer interaction such that the computer interface understands what the user is saying and doing, and the user can simply behave. We believe that natural interaction can provide better learnability, flexibility, memorability, convenience and efficiency, but further user studies are needed to investigate this belief.
...
The proposed work is about developing an intelligent multimodal interface for a tabletop display for the urban search and rescue (USAR) application. Emergency response in crisis management requires strategic assessment of a large volume of complex information. Access to the information by crisis management teams in the command center can be facilitated by various human-computer interfaces. However, these interfaces are hard to use, require a lot of training, and often impede team work \cite{Sharma03}. Intelligent, natural, and multimodal interfaces have the potential to offload the cognitive load from the users, and hence allow them to concentrate more on the decision-making task. Most USAR tasks rely upon geospatial information which is often presented as maps. As a result, gesture interaction become relevant for this type of interaction. The focus of the work will be on developing a multimodal interface for map interaction with the USAR application acting as our test bed. The goal is to develop an interface development framework that can be generalized to other application domains.  
