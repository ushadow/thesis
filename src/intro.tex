...
\textit{flow}, \textit{binding}, and \textit{nature}.
...
they are related to how the user interface should respond to gesture events. The nature
dimension is very similar to the taxonomies mentioned in other related work,
and hence, will be explained further below. However,
the form and the flow dimensions are the focus of this thesis.

\subsubsection{Wobbrock's Nature Dimension}
Along the nature dimension, Wobbrock et al. divide gestures into four
categories: symbolic, physical, metaphorical and abstract. These
categories have some overlap with Pavlovi\'{c}'s taxonomy, but the latter is
more comprehensive.

Figure~\ref{fig:taxonomy} shows a hierarchical classification in the nature
dimension I summarized based on Pavlovi\'{c}'s taxonomy. First, gestures
are \textit{purposive} movements. The distinction is important for a natural interface because a
...
recognition system than the form and the flow dimensions are.
Table~\ref{tab:taxonomy} shows the categories in these two dimensions.
...
The form dimension informs us what different features we need to consider to
differentiate gestures.
...
\begin{itemize}
 ...
 \item Feature extraction
  ...
  \item Gesture recognition
    ...
    simultaneous recognition and segmentation.  With user dependent
    training and testing, the system achieves an average $F_1$ score of 0.805
    (0.93 for discrete flow gestures and 0.68 for continuous flow gestures) on the YANG dataset.
    ...
    real-time interaction with different gesture flows.
