...
an extension of conditional random field (CRF), for gesture recognition, and
...
random field (LDCRF) that is able to perform sequence labeling and
...
filtering to make it work to the unbounded input domain. However, their aproach
still incurrs some delay, e.g., one to four secondes in their experiments. For
real-time interaction, 0.1 second is about the limit for having the user feel
that the system is reacting instantaneously. We may relax this response a bit,
but 1.0 second is about the limit for the user's flow of thought to stay
uninterruped \cite{card91}.  Also they only focused on communicative
gestures and they assumed no unintentional gestures in the input data.

Another limitation of CRF-based models is that training for these modals is much
more computatoinally expensive and converges much slower than those of HMM 
\cite{lafferty01}. To combine the benefits of both HMM-based model and the 
...
the discriminative power of our model, we use discriminative training methods \cite {chang12}.
...
objective function that reflects the degree of confusion; second, ajust the
...
method. We will also compare the results with those when no discriminative
training is used and those with CRF-based models.
...
training examples from the users, and how well the system can perform 
