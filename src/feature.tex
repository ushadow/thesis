...
$\underline{x}_t^{\text{xsens}}$ at every time frame $t$.

However, IMUs cannot provide position information. The evaluation in
Table~\ref{tab:comp-motion-feature} (first and third columns) shows that adding
position information can improve recognition accuracy further for path gestures. With the Kinect
sensor data from the ChAirGest dataset and using the gesture salience based hand
tracking method, I extract the position of a gesturing hand in $(x, y, z)$
coordinates relative to the shoulder center joint to form a 3-dimensional vector
$\underline{x}^\text{kinect}_t$ (should center joint position from the Kinect
